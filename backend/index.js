...
//sistemas
app.get('/user', (req,res)=>{
  let perfis = req.body.setor
  let qr = `select * from cadastro where perfis like ${perfis}`
...
/*
app.get('/user/:id',(req,res)=>{
    console.log(req.body, 'getusuario')
    let nome = req.body.nome
    let login = req.body.login
    let email = req.body.email
    ...
    perfis = setor
    perfis = tipodeusuario
    perfis = perfildeusuario
    perfis = funcao



    //let qr = `select from cadastro(nome, login, email, perfis)`;
    //let qr = `select from cadastro where perfis like ${perfis}`;
    db.query(qr,(err,result)=>{
        if(err){
            console.log(err)
